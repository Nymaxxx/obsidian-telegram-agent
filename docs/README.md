...
| [Bridge spec](bridge-spec.md) | Build specification for the new bridge: what Takopi's retirement takes with it, new capabilities (checkpoints/undo, budget guard, scheduler, skills), security hardening set, env contract, phased build plan. Telegram support is dropped — the sole client is the PWA below |
| [Web client spec](web-client-spec.md) | The bridge's only client: Android-first PWA workbench — browsable multi-session sidebar, share-sheet capture, voice, Web Push, visual diff/undo, dashboards — plus the exposure model behind Coolify + Authentik forward-auth |
