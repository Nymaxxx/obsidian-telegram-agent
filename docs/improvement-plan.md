...
> **Update (July 2026): option C is now the active direction, and the question it answered has changed shape** — the Stage 3 feature appetite materialized, and then the transport itself fell away. Telegram is blocked/degraded in RU and the stack is moving to a home network that cannot poll it, so the bridge is no longer a *Telegram* bridge: **support is dropped for now** and the sole client is a self-hosted PWA on the operator's own domain ([web-client-spec.md](web-client-spec.md)). That retires this whole comparison table — options B and D are Telegram bridges, and A ("stay on Takopi") has nothing left to stay on. The build specification lives in [bridge-spec.md](bridge-spec.md); Takopi remains the shipped default until its Phase 7 cutover, which retires Takopi and the VPS together.
...
> **Update (July 2026): decided in the opposite direction** — the stack moves *toward* self-hosting, not away from it: onto the operator's **home server under Coolify**, published behind their own edge (Traefik terminates TLS, Authentik authenticates) as tier A `agent.app.syntexia.ru`. **The VPS is retired entirely** at cutover: it existed to run this stack, and the last thing that needed it specifically — polling Telegram from a network where that works — left with the [dropped Telegram transport](bridge-spec.md#goals-and-non-goals). The app-side contract, the phase impact and the new risks are in [bridge-spec — Deployment target](bridge-spec.md#deployment-target-the-home-server-under-coolify). Managed Agents stay on Watch, and the case for them is weaker rather than stronger — the vault lives on hardware the operator owns, next to the SSO that guards it.
